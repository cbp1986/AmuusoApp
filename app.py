...
def generate_offer(input_file, tarief, taal, include_stock, lego_mode, discount=None):
    ...
    # Kolomnamen aanpassen voor LEGO-modus
    if lego_mode:
        ...
        df = df.fillna("")
        df = df[df["voorraad"] != "0"]  # Alleen artikelen met voorraad
    ...
    # Promo korting toepassen
    if discount:
        df["Promo"] = df["Prijs/Prix"].astype(float) * (1 - discount / 100)
    
    # Kolommen ordenen
    df = df[["Amuuso Ref", "e-com omschr nl", "e-com omschr fr", "GTIN", "Prijs/Prix", "SRP", "Stock"]]
    
    # Opslaan naar Excel in geheugen
    output = BytesIO()
...
if uploaded_file and st.button("Offerte genereren"):
    output = generate_offer(uploaded_file, tarief, taal, include_stock, lego_mode, discount)
    if output:
